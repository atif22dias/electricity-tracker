...
function App() {
  ...
  // ⚡ SLAB BILLING (your logic)
  const calculateBill = (units) => {
    if (units <= 100) return units * 3.96;

    if (units <= 300)
      return 100 * 3.96 + (units - 100) * 10.8;

    if (units <= 500)
      return 100 * 3.96 + 200 * 10.8 + (units - 300) * 15.03;
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6 flex justify-center">
      <div className="w-full max-w-3xl">
        ...
        <p className="text-slate-400 mb-6">
          Electricity tracker with slab billing
        </p>
        ...
        <div className="bg-slate-800 p-4 rounded-2xl mb-6 space-y-3">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setBillInput(
                calculateBill(num(e.target.value)).toFixed(0)
              );
            }}
          />
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setKwhInput(
                estimateKwh(num(e.target.value)).toFixed(2)
              );
            }}
          />
        ...
        <div className="space-y-3">
          ...
          {list.map((item) => {
            const kwh =
              item.type === "fixed"
                ? (item.watt * 24 * 30) / 1000
                : (item.watt * item.hours * (item.days || 30)) / 1000;
